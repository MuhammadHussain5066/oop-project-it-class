...
let students: string[] = []; // Array to store student names
let staff: string[] = []; // Array to store staff names

function addStudent(name: string) {
  if (!students.includes(name)) {
    students.push(name);
    console.log(`Welcome, ${name}!`); // Welcome message for new student
  } else {
...
function addStaff(name: string) {
  if (!staff.includes(name)) {
    staff.push(name);
    console.log(`Welcome aboard, ${name}!`); // Welcome message for new staff
  } else {
    console.log(`Welcome back, ${name}! Nice to see you again.`);
  }
}
